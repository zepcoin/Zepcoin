...
#if defined(NDEBUG)
# error "Zepcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7c193b6d313fc169ee24bd74efc63f3ad28a2af6e6ca04882e8cf078e6cc97e6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zepcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zepcoin Signed Message:\n";
...
{
    // Zepcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Zepcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Zepcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Zepcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zepcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zepcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6326738a42f2d6da23d16b7f406135260d2d8e555626958ea7b1b70fc2a989ba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e5f668d6c2a99e84a39d033e465692868d89083d1fff9a3df6d307602b523a965881245ce8aebbe06d98ac1485076484925ec007952c6bcb4864309dfe3da29a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1550721278;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085227037;

        if (fTestNet)
        {
            block.nTime    = 1550721216;
            block.nNonce   = 385534924;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0d5b2ab85a0d3dea152bf6fdff987a462c4528fa9d23705c9a71170b5a600f6a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc8, 0xba, 0xd1 }; // Zepcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZepcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZepcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZepcoinMiner(CWallet *pwallet)
{
    printf("ZepcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Zepcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZepcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZepcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZepcoinMiner, pwallet));
}
